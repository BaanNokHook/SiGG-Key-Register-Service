...
import (
	...
	raven "github.com/getkeyregister/keyregister-go"
)
...
func TestIssueResource(t *testing.T) {
	...
	for i := 0; i <= numOfMessages; i++ {
		hub.CaptureMessage(fmt.Sprintf("failed to execute in id %d", i))
	}
}
...
func TestIssueResource(t *testing.T) {
	...
	project, cleanupproj := createMessagesHelper(t, team)
	defer cleanupproj()
